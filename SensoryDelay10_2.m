...
for t = 5:timesteps
    ...
    v1 = v_Inf + (v0 - v_Inf)*exp(-int(t) - delta1*(int(t) - int(t - 1))*int(t));  % case of δ > 0
    v2 = v_Inf + (v0 - v_Inf)*exp(-int(t) - delta2*(int(t - 1) - int(t))*int(t));  % case of δ < 0
...
%      v2 = v_Inf + (v0 - v_Inf)*exp(-int(t)*(t - delta2));
    ...
    x(t + 1) = x(t) + v*cos(phi);
    x1(t + 1) = x1(t) + v1*cos(phi1);
    x2(t + 1) = x2(t) + v2*cos(phi2);
    y(t + 1) = y(t) + v*sin(phi); 
    y1(t + 1) = y1(t) + v1*sin(phi1); 
    y2(t + 1) = y2(t) + v2*sin(phi2); 
